export class float3x2 {
    ...
    applyTo(canvas: CanvasRenderingContext2D) {
        canvas.setTransform(
            this.m11, this.m12,
            this.m21, this.m22,
            this.m31, this.m32);
    ...
    animate(to: float3x2, duration: number) {
        return AnimationManager.linear(duration, rate => {
            this.assign(float3x2.lerp(this, to, duration));
        });
    }
...
export class AnimationManager {
    static linear(duration: number, reportCallback: (completeRate: number) => void) {
        if (duration < 0) throw new Error("duration should never less than 0.");

        let then = new Date().getTime();

        return new Promise<void>(r => {
            function tick() {
                let elasped = new Date().getTime() - then;
                let rate = elasped / duration;
